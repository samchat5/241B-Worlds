...
#pragma config(Motor,  port4,           RightMiddle,   tmotorVex393_MC29, openLoop, reversed, encoderPort, None)
...
void displaysensors(){
	...
	/*
	displayLCDPos(1,0);
	displayNextLCDString("Enco: ");
	displayLCDPos(1,6);
	displayNextLCDNumber(SensorValue[LMS]);
	*/
}//end displaysensors
//turn
...
void move(int dis,int inpower=0,int movepower=127,int indist=dis){//dis is desired movement in inches, inpower sets power of intake 127 in & -127 out (defualts to 0 if not specified)
	//SensorValue[LM]=0;
	//SensorValue[RM]=0;
	...
	int dl=-1*SensorValue[LMS]+(dis/(3.14159265359*4))*392;//dis converted to encoder units + initial left encoder value
	int dlindist=-1*SensorValue[LMS]+(indist/(3.14159265359*4))*392;
	//int dr=SensorValue[RMS]+(dis/(3.14159265359*4))*392; //dis converted to encoder units + initial right encoder value
	if(lor==1){
		while(-1*SensorValue[LMS]<dl-500){//more then half a wheel to dis
			displaysensors();
			//move motors forward
			motor[RightFrontBack] = movepower;motor[RightMiddle] = movepower;
			motor[LeftFrontBack] = movepower;motor[LeftMiddle] = movepower;
			if(-1*SensorValue[LMS]<dlindist){motor[Intake] = inpower;}else{motor[Intake]=0;}
		}//end while values arnt within 2*PI of dis
		//break
			motor[LeftFrontBack] = -1*movepower*0.0394;motor[LeftMiddle] = -1*movepower*0.0394;
			motor[RightFrontBack] = -1*movepower*0.0394;motor[RightMiddle] = -1*movepower*0.0394;
			motor[Intake] = inpower;
			wait1Msec(100);

		//end break
		while(-1*SensorValue[LMS]<dl){//almost at dis
			displaysensors();
			motor[LeftFrontBack] = movepower*0.2362;motor[LeftMiddle] = movepower*0.2362;
			motor[RightFrontBack] = movepower*0.2362;motor[RightMiddle] = movepower*0.2362;
			if(-1*SensorValue[LMS]<dlindist){motor[Intake] = inpower;}else{motor[Intake]=0;}
		}//end while values are almost at dis
		while(-1*SensorValue[LMS]>dl){//after dis
			displaysensors();
			//move motors back
			motor[LeftFrontBack] = -1*movepower*0.2362;motor[LeftMiddle] = -1*movepower*0.2362;
			motor[RightFrontBack] = -1*movepower*0.2362;motor[RightMiddle] = -1*movepower*0.2362;
			if(-1*SensorValue[LMS]>dlindist){motor[Intake] = inpower;}else{motor[Intake]=0;}
	...
	if(lor==-1){
		while(-1*SensorValue[LMS]>dl+500){//more then half a wheel to dis
			displaysensors();
			//move motors back
			motor[LeftFrontBack] = -1*movepower;motor[LeftMiddle] = -1*movepower;
			motor[RightFrontBack] = -1*movepower;motor[RightMiddle] = -1*movepower;
			if(-1*SensorValue[LMS]>dlindist){motor[Intake] = inpower;}else{motor[Intake]=0;}
		}//end while values arnt within 2*PI of dis
			displaysensors();
		//break
			motor[LeftFrontBack] = movepower*0.0394;motor[LeftMiddle] = movepower*0.0394;
			motor[RightFrontBack] = movepower*0.0394;motor[RightMiddle] = movepower*0.0394;
			motor[Intake] = inpower;
			wait1Msec(100);
		//end break
		while(-1*SensorValue[LMS]>dl){//almost at dis
...
task autonomous(){
	...
	if(mode==9){//auto 9 Red, Ball
		//setupGyro(); done in preauton
		move(-38,127,90);//distance between the starting positon and the easy cap			(Works?)
		motor[Intake]=127;
		wait1Msec(1000);

		move(38,127,90);//move back to starting position
		heading(-900,2,0);//to align with starting tile toward flag
...
task usercontrol(){
	while(true){//loops always during driver control to constantly check buttons
		...
		if (vexRT[Btn6U] == 1) 						{motor[Intake] = -127;}//out
		if (vexRT[Btn6D] == 1) 						{motor[Intake] = 127;}//in
